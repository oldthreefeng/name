...
恣 欸 皑 捭 蛃 晡 阐 绰 蛏 眵 敕 逴 婥 凑 逮 袋
...
螈 癌 闇 鮟 醠 謷 磴 礅 壕 壑 礁 嬲 嶷 翳 臆 膺
臃 黝 龠 盫 鳌 蹦 璧 礓 礞 黟 彝 癔 鼬
爊 礤 巅 疆 嬿 繶 鳙
巉 壤 巍 鼯
礴 蠡
鹳 懿 饔
...
`
	...
	G = `
玎 玏 玘 玙 玒 玗 玢 玮 玥 玟 珌 珈 珏 珂 珅 珎 珋 玶
珃 珄 珆 玹 玴 珥 珩 珞 珣 珧 珮 珹 珒 珕 珡 珗 珦 珝 
珬 珚 珜 珱 珘 琎 珺 琅 珵 琈 琁 珳 琋 琂 琲 琛 琚 琬 
珉 琟 琙 琸 瑀 瑍 瑔 瑈 瑅 瑆 瑷 瑨 瑢 瑭 瑸 瑮 瑫 瑧 
瑾 璋 璂 璟 璥 璞 璔 璿
`
)

/*
玎（dīng）：玉类器物撞击时发出的声音。例：玎玲、环佩玎珰
玏（lè）：似玉的美石。
玘（qǐ）：佩玉，一种玉名。
玙（yú）：古代的一种佩玉，寓喻美好。
玒（hóng）：玉名。
玗（yú）：似玉的美石。
玢（bīn、fēn）：玉名，一种玉的花纹。
玮（wěi）：玉名，美好、珍贵、珍爱、珍视之意。
玥（yuè）：传说中的一种神珠。
玟（mín、wén）：玉的纹理。
珌（bì）：佩刀的玉饰。
珈（jiā）：古代妇女显示身份的一种头饰。
珏（jué）：合在一起的两块玉。
珂（kē）：似玉的美石；白玛瑙；古代朝服上的玉带。
珅（shēn）：玉名。
珎（zhēn）：同“珍”。
珋（liǔ）：琉璃。
玶（píng）：玉名。
...
珄（shēng）：金色。
珆（yí）：玉石。
玹（xuán、xián）：玉色；似玉的美石。
玴（yì）：似玉的美石。
珥（ěr）：用珠子或玉石做的耳饰。
珩（héng、háng）：佩玉上面的横玉。
珞（luò）：璎珞，古代一种用珠玉穿成串、戴在颈项上的装饰品。
珣（xún）：玉名
珧（yáo）：蚌蛤的甲壳，古代用作刀、弓上的装饰品。
珮（pèi）：同“佩”
珹（chéng）：美玉，美珠
珒（jīn）：玉名
珕（lì）：古代用贝壳做刀鞘上的装饰。
...
璟（jǐng）：玉的光彩
璥（jǐng）：玉
璞（pú）：未雕琢过的玉石，或指包藏着玉的石头；喻人的天真状态，质朴，淳朴。
璔（zēng）：玉的美貌。
璿（xuán）：同“璇”
*/
